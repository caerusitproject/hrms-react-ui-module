// create-template-modal.jsx (Updated with formatting toolbar)
"use client"
...
export default function CreateTemplateModal({ open, onClose, onSave }) {
  ...
  const insertNewParagraph = (content = '') => {
    ...
    insertFormat('p', newPara); // Reuse insert but for p
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await EmailTemplateAPI.createTemplate(payload);
      onSave(response);
    } catch (err) {
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="lg" fullWidth>
      <DialogTitle style={{ color: theme.colors.text.primary }}>Create New Email Template</DialogTitle>
      <DialogContent>
      ...
      <DialogActions>
        ...
        <Button onClick={handleSubmit} variant="contained" style={{ ...theme.saveBtn }}>Save Template</Button>
      </DialogActions>
    </Dialog>
  );
}